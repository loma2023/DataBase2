...
if (document.title == "التحصيل من العملاء") {
  pages.querySelectorAll("a")[1].classList.add("hoverd")
} else if (document.title == "الدفع للعملاء") {
...
function fetches() {
  ...
  fetch(General_URL)
    ...
    .then((row) => {
      ...
      document.querySelectorAll("section")[0].id = row.length + 1
      showData()
...
function insert_value(id) {
  ...
  let row = document.querySelectorAll("section")[0].id
  ...
  else if (name === "اسم العميل") { alert("اختر اسم العميل"); return }
  else if (amountdebit === "") { alert("برجاء ادخال المبلغ"); return }

  if (id == "debit") { debit = "الصندوق"; credit = "العملاء"; subdebit = "الصندوق"; subcredit = name; nu = '"A"' }
  else if (id == "credit") { debit = "العملاء"; credit = "الصندوق"; subdebit = name; subcredit = "الصندوق"; nu = '"B"' }
